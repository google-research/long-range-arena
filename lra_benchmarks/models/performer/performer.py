...
class PerformerSeq2SeqBlock(nn.Module):
  """Performer encoder-decoder layer."""
  ...
  def apply(self,
            ...
            attention_fn_cls=_DEFAULT_ATTENTION_FN_CLS,
            attention_fn_kwargs=None):
    """Applies PerformerSeq2Seq module.
    ...
    x = nn.SelfAttention(
        ...
        deterministic=deterministic,
        cache=cache,
        attention_fn=attention_fn(qkv_dim // num_heads, unidirectional=True))
    x = nn.dropout(x, rate=dropout_rate, deterministic=deterministic)
    x = x + targets
    ...
    y = nn.SelfAttention(
        ...
        dropout_rate=attention_dropout_rate,
        deterministic=deterministic,
        attention_fn=attention_fn(qkv_dim // num_heads, unidirectional=False))
...
class PerformerSeq2SeqDecoder(nn.Module):
  """Performer Model Decoder for sequence to sequence translation."""
...
class PerformerSeq2Seq(nn.Module):
  """Performer Model for sequence to sequence translation."""
  ...
  def apply(self,
    ...
    Args:
      ...
      attention_fn_cls: Attention function key or callable.
      attention_fn_kwargs: Keywords to pass to `attention_fn_cls`.
      vanilla_decoder: Boolean, set to True to use vanilla transformer decoder.
    ...
    logits = PerformerSeq2SeqDecoder(
        encoded,
        src_padding_mask,
        targets,
  ...
  # The following two methods allow us to run the trained Performer in
